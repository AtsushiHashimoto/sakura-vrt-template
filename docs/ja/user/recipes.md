...
> **`sudo shutdown -h now` とコントロールパネルの「停止」について**
> さくらのクラウドの VRT サーバーは ACPI 対応のため、OS 上で `sudo shutdown -h now` を実行すると
> コントロールパネル上でもサーバーが「停止」（OFF）状態になります。
> GPU 課金が止まり、システムディスク（`/workspace` のデータ）は保持されます。
